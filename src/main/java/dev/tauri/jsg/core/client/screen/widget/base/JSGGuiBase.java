package dev.tauri.jsg.core.client.screen.widget.base;
...
import static dev.tauri.jsg.core.client.screen.util.GuiHelper.drawRect;
...
public class JSGGuiBase extends Screen {
    ...
    protected void translateToCenter() {
        graphics.pose().translate((width - imageWidth) / 2f, (height - imageHeight) / 2f, 0);
    }
    ...
    protected void drawTextBox(EditBox tf) {
        int x = tf.getX() - 2;
        int y = tf.getY() - 2;
        int x2 = tf.getX() + tf.getWidth();
        int y2 = tf.getY() + tf.getHeight();

        drawRect(x - 1, y - 1, x2 + 1, y2 + 1, frameColor);
        drawRect(x, y, x2, y2, 0xFF1D2026);

        tf.render(graphics, mouseX, mouseY, partialTick);
    }
    ...
    protected EditBox createTextField(int x, int y, int width, int maxLength, String defaultText) {
        ...
        EditBox tf = new EditBox(font, x1 + 3, y1 - 1, width + 2, 10 - 1, Component.literal(""));

        tf.setMaxLength(maxLength);
        tf.setBordered(false);
        tf.setValue(defaultText);
