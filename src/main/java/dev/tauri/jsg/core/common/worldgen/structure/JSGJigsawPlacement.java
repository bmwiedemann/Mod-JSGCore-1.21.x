...
/**
 * Original created by Povstalec in Stargate Journey
 * We are allowed to copy this code and modify this code for our own usage in JSG
 */

@SuppressWarnings("all")
public class JSGJigsawPlacement extends JigsawPlacement {
    public static Optional<Structure.GenerationStub> addPieces(Structure.GenerationContext context, Holder<StructureTemplatePool> startPool, Optional<ResourceLocation> startJigsawName,
        ...
        if (structurepoolelement == EmptyPoolElement.INSTANCE)
            ...
            if (startJigsawName.isPresent()) {
                ...
                if (optional.isEmpty()) {
                    JSGCore.logger.error("No starting jigsaw {} found in start pool {}", resourcelocation, startPool.unwrapKey().get().location());
                    return Optional.empty();
                }
            ...
            PoolElementStructurePiece poolelementstructurepiece = new PoolElementStructurePiece(structuretemplatemanager, structurepoolelement, blockpos1, structurepoolelement.getGroundLevelDelta(), rotation, structurepoolelement.getBoundingBox(structuretemplatemanager, blockpos1, rotation), net.minecraft.world.level.levelgen.structure.structures.JigsawStructure.DEFAULT_LIQUID_SETTINGS);
